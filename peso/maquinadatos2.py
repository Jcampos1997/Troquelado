...
class Maquinadatos2(Screen):
    def __init__(self, **kwargs):
        super(Maquinadatos2, self).__init__(**kwargs)
        print('inicie')
        # connect DB
    ...
    def inicioprocesom1(self):
        ...
        if len(ofmaq) == 0 or len(opmaq) == 0:
            ...
            consulta = "SELECT id_parada FROM parada where id_parada='2'"
            ...
            # consultax = "INSERT into auditoria(of,canuto,rollo,desperdicio,maquina,id_maquina)values ('"+ofmaq+"','0','1','0','103','"+str(id_maquina)+"')"
            # re1.execute(consultax)
            act_parada = "UPDATE parada set estado='activo' where id_parada='2'"
            re1.execute(act_parada)
            luc.commit()
            count = re1.rowcount
            re1.close()
            print("Parada #2 iniciada")
            self.parent.current = 'rollod'
            # Clock.schedule_once(self.setup, 3)


Config.set('graphics', 'resizable', True)
Builder.load_file('maquinadatos2.kv')
