    ...
    def cerrar_canuto_primero(self):
        ...
        while 1:
            ...
            if not data:
                ...
                consulta2 = "SELECT of_rollos FROM maquina_of where maquina='102' and estado='nuevo' order by id desc limit 1 "
                re6.execute(consulta2)
                rowse = re6.fetchall()
                re6.close()

                # extraigo el id de la máquina
                consulta_id = "SELECT id FROM maquina_of where maquina='102' and estado='nuevo' order by id desc limit 1 "
                ...
                for row in rowse:
                    ...
                    consulta = "INSERT into peso_canuto(fecha_creacion, ofab,codigo,id_maquina_of,peso)values ('" + \
                        dt+"','"+yytt2+"','"+codi+"','" + \
                        str(id_maquina_of)+"','"+str(peso)+"')"
    ...
    def cerrar_rollo_ultimo(self):
        ...
        while 1:
            ...
            if not data:
                ...
                re21 = luc.cursor()
                # cargo of a guardar en canuto
                consulta2 = "SELECT of_rollos FROM maquina_of where maquina='102' order by id desc limit 1 "
                ...
                consulta_id = "SELECT id FROM maquina_of where maquina='102' order by id desc limit 1 "
                ...
                for row in rowse:
                    ...
                    n = random.randint(100, 99999)
                    codif = yytt2+"_"+str(n)
                    quy = "select codigo from peso_canuto where ofab='" + \
                        yytt2+"' order by id_peso_canuto DESC limit 1"
                    cur = luc.cursor()
                    peso = float(y)
                    consulta = "INSERT into peso_materia(fecha_creacion, peso,ofab,codcanuto,codrollo,estado, id_maquina_of)values ('"+dt+"','"+str(
                        peso)+"','"+yytt2+"',"'('+quy+')'",'"+codif+"','habilitado', '"+str(id_maquina_of)+"')"
...
peso_canuto = 0.0  # variable global
...
class Rollo(Screen):
    ...
    # NO USAR
    def cerrar_rollo_ultimo2(self):
        sp1 = lio.cursor()
        qrysp1 = 'SELECT a."DocNum", a."ItemCode", a."ProdName", c."ItmsGrpNam" FROM "SUPRALIVE_PRD"."OWOR" a INNER JOIN "SUPRALIVE_PRD"."OITM" b ON a."ItemCode" = b."ItemCode" INNER JOIN "SUPRALIVE_PRD"."OITB" c ON b."ItmsGrpCod" = c."ItmsGrpCod" where a."DocNum"=249372'
        sp1.execute(qrysp1)
        cds = sp1.fetchone()
        pp = cds[1]
        print(cds)
    ...
    def cerrar_rollo_ultimo(self):
        ...
        while 1:
            ...
            if not data:
                ...
                re21 = luc.cursor()
                # cargo of a guardar en canuto
                consulta2 = "SELECT ordenf FROM of_troquelado where parada='1' order by id desc limit 1 "
                re21.execute(consulta2)
                rowse = re21.fetchall()
                re21.close()
                yytt2 = 0
                ...
                # obtenemos el id de maquina_of
                codigo_id = luc.cursor()
                consulta_id = "SELECT id_parada FROM parada where id_parada='1' order by id desc limit 1 "
                codigo_id.execute(consulta_id)
                id = codigo_id.fetchone()
                id_maquina_of = id[0]
                codigo_id.close()
                ...
                for row in rowse:
                    ...
                    cur = luc.cursor()
                    peso = float(y)
                    consulta = "INSERT into rollo_troquelado(codigo, peso) values ('"+yytt2+"','"+str(
                        peso)+"')"
                    cur.execute(consulta)
                    luc.commit()
                    count = cur.rowcount
                    ...
                    re23 = luc.cursor()
                    # impresion
                    consulta = "select a.id_peso_mat,b.of_rollos, b.maquina, b.operador, a.codrollo,a.peso from peso_materia a, maquina_of b where a.ofab=b.of_rollos and b.estado='nuevo' and b.maquina='102' order by id_peso_mat desc limit 1"
                    re23.execute(consulta)
                    cd = re23.fetchone()
                    ofs = cd[1]
                    print(ofs)
    ...
    # ESTA FUNCION SE EJECUTA EN rollo.kv UNA VEZ SE DIO CLICK EN PESAR CANUTO
    def comenzar(self):
    ...
    def finalizo(self):
        self.parent.current = 'finalizarx'
...
# extraigo ultima of no finalizada de la maquina 102, pregunto si existo
        ...
        conx = "SELECT ordenf FROM of_troquelado where parada='2' and estado not in ('finalizado') order by id desc limit 1 "
        re2.execute(conx)
        cd = re2.fetchone()
        if cd == None:
        ...
        cony = "SELECT ordenf FROM of_troquelado where parada='3' and estado not in ('finalizado') order by id desc limit 1 "
        re3.execute(cony)
        cd = re3.fetchone()
        if cd == None:
...
# FIN BOTONES DE MAQUINAS
        ...
        consultaop = "SELECT usuario FROM of_troquelado where parada='1' order by id desc limit 1"
        ...
        if cd == None:
            ...
            re5 = luc.cursor()
            print(ofrollo)
            consulta = "SELECT canuto,rollo FROM of_troquelado where of='"+ofrollo+"' "
            print(consulta)
    ...
    def pesar_canuto_primero(self):
        ...
        try:
            ...
            popup = Popup(title='Alerta',
                          content=Label(
                              text='Error, pese el canuto de nuevo!'),
                          size_hint=(None, None), size=(410, 150))
            popup.open()
    ...
    def pesar_rollo_ultimo(self):
        ...
        try:
            self.cerrar_rollo_ultimo2()
            popup = Popup(title='Rollo último',
                          content=Label(
                              text='Útimo rollo pesado correctamente!'),
                          size_hint=(None, None), size=(410, 150))
            popup.open()
        except:
            popup = Popup(title='Alerta!',
                          content=Label(text='Error, pese el rollo de nuevo!'),
                          size_hint=(None, None), size=(410, 150))
            popup.open()

# Extraigo peso CANUTO -------------------------------------------
    def extraer_pesorc(self):
        try:
            btncap2 = self.ids.btncap2
            btncapturar = self.ids.btncapturar
...
# Extraigo peso CORTE-------------------------------------------
    ...
    def accion(self):
        try:
            ...
            popup = Popup(title='Alerta!',
                          content=Label(
                              text='Error, por favor, pese de nuevo!'),
                          size_hint=(None, None), size=(410, 150))
            popup.open()
            ...
            while 1:
                ...
                if not data:
                    ...
                    for row in rowse:
                        ...
                        consulta = "INSERT into peso_materia(fecha_creacion, peso,ofab,codcanuto,codrollo,estado,finca, id_maquina_of)values ('"+dt+"','"+str(
                            peso)+"','"+yytt2+"',"'('+quy+')'",'"+codif+"','habilitado','finca', '"+str(id_maquina_of)+"')"
                        ...
                        # impresion
                        re15 = luc.cursor()
                        consulta = "select a.id_peso_mat,b.of_rollos, b.maquina, b.operador, a.codrollo,a.peso from peso_materia a, maquina_of b where a.ofab=b.of_rollos and b.estado='nuevo' and b.maquina='102'  and a.finca='finca' order by id_peso_mat desc limit 1"
                        ...
                        ftotal = fline1+fline1a+fline1b+fline1c+fline1d+fline2+fline3+fline5+fline4
                        os.remove("maquina1.txt")
                        f = open("maquina1.txt", "a+")
                        f.write(ftotal)
                        f.close()
                        os.system('lpr "maquina1.txt"')
                    ser.close()
                    break
        except:
            popup = Popup(title='Alerta!',
...
# Extraigo peso ROLLO FINCA -------------------------------------------
    ...
    def extraer_pesord(self):
        try:
            ...
            while 1:
                ...
                if not data:
                    ...
                    for row in rowse:
                        ...
                        re19 = luc.cursor()
                        consulta = "INSERT INTO peso_desperdicio(fecha,ofab,peso,id_maquina_of)values ('"+dt+"','"+yytt2+"','"+str(
                            peso)+"','"+str(id_maquina_of)+"')"
            ...
            popup = Popup(title='Alerta!',
                          content=Label(
                              text='Error, pese el picado de nuevo!'),
                          size_hint=(None, None), size=(410, 150))
            popup.open()
...
# peso rollo
        ...
        try:
            ...
            while 1:
                ...
                if not data:
                    ...
                    consulta_id = "SELECT id FROM maquina_of where maquina='102' order by id desc limit 1 "
                    codigo_id.execute(consulta_id)
                    consulta_id = codigo_id.fetchone()
                    id_maquina_of = consulta_id[0]
                    codigo_id.close()
    ...
    def printer(self):
        ...
        fline1 = "\nÓRDEN DE FAB: "+cd[1]+"     MÁQUINA:PARADA 2\n\n"
        fline1a = "CÓDIGO PRODUCTO: "+cds[1]+"\n"
        fline1b = "PRODUCTO: "+cds[2]+"\n"
        fline1c = "GRUPO: "+cds[3]+"\n\n"
        fline1d = "PESO: "+str(peso_fin)+"\n\n"
