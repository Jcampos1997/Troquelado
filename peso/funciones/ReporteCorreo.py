...
class ReporteCorreo():
    ...
    def imprimirReporte(self):   
        ...
        ultima_orden_fabricacion = "SELECT ofab FROM peso_materia pm INNER JOIN maquina_of mo ON "\
                                   ...
                                   "LIKE '{}%' AND mo.maquina LIKE '%102%' ORDER BY id_peso_mat DESC limit 1 ".format(str(fecha_actual))
        ...
        ultima_consulta.close()
        ultima_consulta2.close()
        ...
        peso_total = 0.0
        canvas2.save()
        ServerCorreo.enviarCorreoReporte()
